...
f <- 'data/prepData_nooffset_buffPol10_hole2_allsites.RDS'          # check this !
prepData <- readRDS(f)
length(prepData)
...
datOut <- snow::clusterApply(cl, prepData, function(pd) {
  ...
  sessionTag <- 'prepData_nooffset_buffPol10_hole2_allsites'        # check this !
...
save(datOut,
     file=paste0('/home/tcrnbgh/Scratch/quarry_data/',
                 'prepData_nooffset_buffPol10_hole2_allsites',       # check this !
                 'int_times.RDS'))
print('done!')
