...
f <- 'data/prepData_alllocs_norm_maxdiff01_smpper0.RDS'          # check this !
prepData <- readRDS(f)
length(prepData)

print('done!')
print('truncating prepData...')
prepData <- prepData[483:643]                                     # check this !
...
datOut <- snow::clusterApply(cl, prepData, function(pd) {
  ...
  sessionTag <- 'prepData_alllocs_norm_maxdiff01_smpper0'        # check this !
...
save(datOut,
     file=paste0('/home/tcrnbgh/Scratch/quarry_data/',
                 'prepData_alllocs_norm_maxdiff01_smpper0',       # check this !
                 'int_times.RDS'))
print('done!')
